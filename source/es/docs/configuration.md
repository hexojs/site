...
| `pretty_urls`                | Reescritura de las variables del [`enlace permanente`](permalinks.html) para embellecer las URL                               |                             |
...
`updated_option` contral el valor de `updated` cuando no se indica uno en el frontispicio:
...
`use_date_for_updated` fue eliminado en v7.0.0+. En su lugar puedes usa `updated_option: 'date'`.
{% endnote %}
...
La ruta a un fichero de configuración personalizado puede ser especificada incluyendo la opción `--config` a los comandos `hexo` con una ruta a un fichero de configuración alternativo en formato YAML o JSON, o una lista separada por comas (sin espacios) de multiples ficheros YAML o JSON.
...
Al usar multiples ficheros de configuración estos combinan los ajustes en  `_multiconfig.yml`. Los valores posteriores tienen precedencia. Funciona con cualquier número de ficheros JSON y YAML con objetos de profundidad arbitraria. Ten en cuenta que **los espacios no están permitidos en la lista**

Por ejemplo, en el ejemplo superior, si `foo: bar` se encuentra en `custom.yml`, pero `"foo": "dinosaur"` está en `custom2.json`, `_multiconfig.yml` contendrá `foo: dinosaur`.
...
El fichero debería ser situado en el directorio del sitio, tanto `yml` como `json` son válidos. `theme` en `_config.yml` debe ser configurado para que Hexo lea `_config.[theme].yml`
