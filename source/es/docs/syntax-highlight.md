...
> Consejo: Cuando `line_number` está desactivado con `false`, `wrap` es falso y `hljs` es `true`, puedes entonces usar `highlight.js` como [estilo de tema](https://github.com/highlightjs/highlight.js/tree/master/src/styles) directamente en tu sitio.
