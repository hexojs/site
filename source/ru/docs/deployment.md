...
Перед первой публикацией сайта, нужно изменить некоторые настройки в `_config.yml`. Правильные параметры развертывания должны иметь поле `type`. Например:
...
Вы можете использовать несколько сервисов размещения. Hexo будет выполнять все в том порядке, каакой указан в файле.
...
`root` | Корневой каталог на удаленном хосте |
`port` | Порт | 22
...
`user` | Имя пользователя |
`pass` | Пароль |
`remote` | Корневой каталог на удаленном хосте | `/`
`port` | Порт | 21
