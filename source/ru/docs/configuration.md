...
`public_dir` | Папка публикации. Здесь хранится сгененированный сайт | `public`
`tag_dir` | Папка с тэгами | `tags`
`archive_dir` | Папака с архивами | `archives`
`category_dir` | Папка с каьегориями | `categories`
...
Параметр | Описание | Умолчание
--- | --- | ---
`date_format` | Формат даты | `MMM D YYYY`
...
Параметр | Описание
--- | ---
`theme` | Имя темы. `false` отклюдчает применение тем
