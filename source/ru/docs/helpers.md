...
Переносит строки длинне указазанного в `length` количества символов. `length` равняется 80 по умолчанию.
...
Обрезает текст после определенного в `length` количества символов. По умолчанию `length` 30 символов.
...
`only` | Строгие локальные переменные. Использовать только только указанные в шаблоне переменные `locals`. | `false`
...
`transform` | Функция, позволяющая изменить отображаемое имя категории. |
`amount` | Ограничени количества отображаемых тэгов (0 = неограниченно) | 0
