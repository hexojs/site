        ...
        if (assetData.length) {
          try {
            const result = assetData.join().replace(/src=['"](.*?)['"]/gi, (str, value) => str.replace(value, (img) => {
              if (updatePng[img]) return updatePng[img];
              return img;
            }));
